import React from "react";
import ReactDOM from "react-dom";
import { cleanup } from "@testing-library/react";
import { AsyncCombobox } from "../index";
import Enzyme, { mount } from "enzyme";
import Adapter from "enzyme-adapter-react-16";

Enzyme.configure({ adapter: new Adapter() });

// automatically unmount and cleanup DOM after the test is finished.
afterEach(cleanup);

jest.useFakeTimers();

describe("Async Component Events tests", () => {
  it("Simple AsyncCombobox creation", () => {
    /* https://github.com/airbnb/enzyme/issues/1525 */
    document.body.innerHTML = '<div id="destattach"></div>';
    const options = {
      attachTo: document.querySelector("#destattach")
    };

    Object.defineProperties(window.document.documentElement, {
      clientWidth: {
        value: 300,
        writable: false
      }
    });

    const map = {};

    document.addEventListener = jest.fn((event, cb) => {
      map[event] = cb;
    });

    const component = mount(
      <div id="container">
        <button id="outside">click outside</button>
        <AsyncCombobox
          id="1234978"
          labels={{ "cap.placeholder": "Search..." }}
          maxDropdownItems={5}
          async={(text, callback) => {
            callback([
              { label: "Grapefruit", value: "Grapefruit" },
              { label: "Mandarin", value: "Mandarin" }
            ]);
          }}
          onChangeAfterCharNum={3}
          onSelection={() => {}}
          delay={1}
        />
      </div>,
      options
    );

    //closed
    expect(component.find(".input-box").html()).toEqual(
      '<div class="input-box"><div class="input-container" style="width: 200px;"><div class="rbc-2b097c-container "><div class="rbc-13srbr-control "><div class="rbc-1hwfws3"><div class="rbc-b8ldur-Input"><input id="rbc-13sxxr-input-1234978" class="rbc-13sxxr-input" type="search" style="box-sizing: content-box; width: 90%; background: center; border: 0px; font-size: inherit; opacity: 1; outline: 0px;" value=""></div></div></div></div><div class="dropdown-menu "><div class="rbc-padding-left20">No items</div></div></div></div>'
    );
  });

  it("Should activate search", () => {
    const component = mount(
      <AsyncCombobox
        id="1234978"
        labels={{ "cap.placeholder": "Search!", "lst.empty": "Empty" }}
        maxDropdownItems={5}
        async={(text, callback) => {
          callback([
            { label: "Grapefruit", value: "Grapefruit" },
            { label: "Mandarin", value: "Mandarin" }
          ]);
        }}
        onChangeAfterCharNum={1}
        onSelection={() => {}}
        delay={0}
      />
    );

    component.instance().getCaptionTextContainerSize = jest.fn(() => 0);
    component.instance().getCaptionTextSize = jest.fn(() => 0);
    component.update();

    let eventObj = { target: { value: "AA" } };
    component.find("input").simulate("change", eventObj);
    expect(component.find(".input-box").html()).toEqual(
      '<div class="input-box"><div class="input-container" style="width: 200px;"><div class="rbc-2b097c-container "><div class="rbc-13srbr-control "><div class="rbc-1hwfws3"><div class="rbc-b8ldur-Input"><input id="rbc-13sxxr-input-1234978" class="rbc-13sxxr-input" type="search" style="box-sizing: content-box; width: 90%; background: center; border: 0px; font-size: inherit; opacity: 1; outline: 0px;" value="AA"><div class="pull-right"><a class="clear"><img style="width: 15px; height: 15px;" src="data:image/png;base64,"></a></div></div></div></div></div><div class="dropdown-menu "><div class="rbc-padding-left20">Empty</div></div></div></div>'
    );

    eventObj = { target: { value: "AAA" } };
    component.find("#rbc-13sxxr-input-1234978").simulate("change", eventObj);
    component.update();

    jest.runAllTimers();

    expect(component.find(".dropdown-menu").html()).toEqual(
      '<div class="dropdown-menu open"><ul id="rbc-menu-button-dropdown-list-1234978" class="dropdown-menu inner" style="max-height: 130px;"> <li class="noselect"><a class="rbc-selection"><span class="rbc-icon"></span>Grapefruit</a></li><li class="noselect"><a class="rbc-selection"><span class="rbc-icon"></span>Mandarin</a></li></ul></div>'
    );
  });

  it("Should activate search - empty result", () => {
    const component = mount(
      <AsyncCombobox
        id="1234978"
        labels={{ "cap.placeholder": "Search!", "lst.empty": "Empty" }}
        maxDropdownItems={5}
        async={(text, callback) => {
          callback([]);
        }}
        onChangeAfterCharNum={1}
        onSelection={() => {}}
        delay={0}
      />
    );

    component.instance().getCaptionTextContainerSize = jest.fn(() => 0);
    component.instance().getCaptionTextSize = jest.fn(() => 0);
    component.update();

    let eventObj = { target: { value: "AA" } };
    component.find("input").simulate("change", eventObj);
    expect(component.find(".input-box").html()).toEqual(
      '<div class="input-box"><div class="input-container" style="width: 200px;"><div class="rbc-2b097c-container "><div class="rbc-13srbr-control "><div class="rbc-1hwfws3"><div class="rbc-b8ldur-Input"><input id="rbc-13sxxr-input-1234978" class="rbc-13sxxr-input" type="search" style="box-sizing: content-box; width: 90%; background: center; border: 0px; font-size: inherit; opacity: 1; outline: 0px;" value="AA"><div class="pull-right"><a class="clear"><img style="width: 15px; height: 15px;" src="data:image/png;base64,"></a></div></div></div></div></div><div class="dropdown-menu "><div class="rbc-padding-left20">Empty</div></div></div></div>'
    );

    eventObj = { target: { value: "AAA" } };
    component.find("#rbc-13sxxr-input-1234978").simulate("change", eventObj);
    component.update();

    jest.runAllTimers();

    expect(component.find(".dropdown-menu").html()).toEqual(
      '<div class="dropdown-menu open"><div class="rbc-padding-left20">Empty</div></div>'
    );
  });

  it("Should activate search - string result", () => {
    const component = mount(
      <AsyncCombobox
        id="1234978"
        labels={{ "cap.placeholder": "Search!", "lst.empty": "Empty" }}
        maxDropdownItems={5}
        async={(text, callback) => {
          callback(["Grapefruit"]);
        }}
        onChangeAfterCharNum={1}
        onSelection={() => {}}
        delay={0}
      />
    );

    component.instance().getCaptionTextContainerSize = jest.fn(() => 0);
    component.instance().getCaptionTextSize = jest.fn(() => 0);
    component.update();

    let eventObj = { target: { value: "AA" } };
    component.find("input").simulate("change", eventObj);
    expect(component.find(".input-box").html()).toEqual(
      '<div class="input-box"><div class="input-container" style="width: 200px;"><div class="rbc-2b097c-container "><div class="rbc-13srbr-control "><div class="rbc-1hwfws3"><div class="rbc-b8ldur-Input"><input id="rbc-13sxxr-input-1234978" class="rbc-13sxxr-input" type="search" style="box-sizing: content-box; width: 90%; background: center; border: 0px; font-size: inherit; opacity: 1; outline: 0px;" value="AA"><div class="pull-right"><a class="clear"><img style="width: 15px; height: 15px;" src="data:image/png;base64,"></a></div></div></div></div></div><div class="dropdown-menu "><div class="rbc-padding-left20">Empty</div></div></div></div>'
    );

    eventObj = { target: { value: "AAA" } };
    component.find("#rbc-13sxxr-input-1234978").simulate("change", eventObj);
    component.update();

    jest.runAllTimers();

    expect(component.find(".dropdown-menu").html()).toEqual(
      '<div class=\"dropdown-menu open\"><ul id=\"rbc-menu-button-dropdown-list-1234978\" class=\"dropdown-menu inner\" style=\"max-height: 130px;\"> <li class=\"noselect\"><a class=\"rbc-selection\"><span class=\"rbc-icon\"></span>Grapefruit</a></li></ul></div>'
    );
  });

  it("Should activate search - clear", () => {
    const component = mount(
      <AsyncCombobox
        id="1234978"
        labels={{ "cap.placeholder": "Search!", "lst.empty": "Empty" }}
        maxDropdownItems={5}
        async={(text, callback) => {
          console.log("call");
          callback([
            { label: "Grapefruit", value: "Grapefruit" },
            { label: "Mandarin", value: "Mandarin" }
          ]);
        }}
        onChangeAfterCharNum={1}
        onSelection={() => {}}
        delay={2}
      />
    );

    component.instance().getCaptionTextContainerSize = jest.fn(() => 0);
    component.instance().getCaptionTextSize = jest.fn(() => 0);
    component.update();

    let eventObj = { target: { value: "AA" } };
    component.find("input").simulate("change", eventObj);
    expect(component.find(".input-box").html()).toEqual(
      '<div class="input-box"><div class="input-container" style="width: 200px;"><div class="rbc-2b097c-container "><div class="rbc-13srbr-control "><div class="rbc-1hwfws3"><div class="rbc-b8ldur-Input"><input id="rbc-13sxxr-input-1234978" class="rbc-13sxxr-input" type="search" style="box-sizing: content-box; width: 90%; background: center; border: 0px; font-size: inherit; opacity: 1; outline: 0px;" value="AA"><div class="pull-right"><a class="clear"><img style="width: 15px; height: 15px;" src="data:image/png;base64,"></a></div></div></div></div></div><div class="dropdown-menu "><div class="rbc-padding-left20">Empty</div></div></div></div>'
    );

    component.find(".clear").simulate("click");

    expect(component.find("input").html()).toEqual(
      '<input id="rbc-13sxxr-input-1234978" class="rbc-13sxxr-input" type="search" style="box-sizing: content-box; width: 90%; background: center; border: 0px; font-size: inherit; opacity: 1; outline: 0px;" value="">'
    );
  });
});
