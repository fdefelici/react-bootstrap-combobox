...
| `trigEvent` | string | You can run these actions: `clear`, `reset`. You need to use an unique id to run the action multiple times. To simplify the creation of an action with an unique id, you can call `Select.TrigEvent.clear()` or  `Select.TrigEvent.reset()`. `clear` deselects all items selected in the past. `reset` resets data removing all previous selections; if data contains selected items, the widget will set them. | none |
